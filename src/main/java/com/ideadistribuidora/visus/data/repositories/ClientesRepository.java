...
public interface ClientesRepository extends JpaRepository<Clientes, Integer>, JpaSpecificationExecutor<Clientes> {
    ...
    @Query("SELECT c FROM Clientes c WHERE c.numeroDeDocumento = :numeroDeDocumento")
    Optional<Clientes> findClienteByNumeroDeDocumento(@Param("numeroDeDocumento")Long numeroDeDocumento);

    Optional<Clientes> findByTipoDeDocumentoAndNumeroDeDocumento(int tipoDeDocumento, Long numeroDeDocumento);
}
