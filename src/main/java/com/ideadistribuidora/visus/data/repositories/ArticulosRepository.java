...
public interface ArticulosRepository extends JpaRepository<Articulos, Integer>, JpaSpecificationExecutor<Articulos> {
        ...
        List<Articulos> findArticulosByIdLinea(Lineas lineas);

        
        List<Articulos> findArticulosByIdProveedor(Proveedores value);

        @Query("SELECT a FROM Articulos a WHERE a.margen_utilidad = :margen_utilidad")
        List<Articulos> findArticulosByMargen_utilidad(@Param("margen_utilidad") BigDecimal margen_utilidad);

        @Query("SELECT a FROM Articulos a WHERE a.codigo_barra= :codigo_barra")
        Optional<Articulos> findArticulosByCodigo_Barra(@Param("codigo_barra") String codigo_barra);
