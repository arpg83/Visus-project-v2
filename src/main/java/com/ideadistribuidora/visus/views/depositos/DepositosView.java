package com.ideadistribuidora.visus.views.depositos;
...
import com.ideadistribuidora.visus.services.DepositosService;
...
@PageTitle("Depositos")
@Menu(icon = "line-awesome/svg/columns-solid.svg", order = 9)
@Route(value = "10/:depositosID?/:action?(edit)")
public class DepositosView extends Div implements BeforeEnterObserver {

    private final String DEPOSITOS_ID = "depositosID";
    private final String DEPOSITOS_EDIT_ROUTE_TEMPLATE = "10/%s/edit";

    private final Grid<Depositos> grid = new Grid<>(Depositos.class, false);
    ...
    private CollaborationBinder<Depositos> binder;

    private Depositos depositos;

    private final DepositosService depositosService;
    GridListDataView<Depositos> dataView;

    public DepositosView(DepositosService depositosService) {
        this.depositosService = depositosService;
        addClassNames("depositos-view");
        ...
        // Configure Grid
        grid.addColumn(createDepositosRenderer()).setHeader("Descripción").setAutoWidth(true);
        dataView = grid.setItems(depositosService.depositosList());
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                UI.getCurrent().navigate(
                        String.format(DEPOSITOS_EDIT_ROUTE_TEMPLATE,
                                event.getValue().getIdDeposito()));
            } else {
                delete.setEnabled(false);
                clearForm();
                UI.getCurrent().navigate(DepositosView.class);
            }
        });

        // Configure Form
        binder = new CollaborationBinder<>(Depositos.class, userInfo);
        ...
        save.addClickListener(e -> {
            ...
            try {
                ...
                binder.writeBean(this.depositos);
                depositosService.update(this.depositos);
                ...
                if (except.getCause().getCause() instanceof SQLException) {
                    ...
                    if (e1.getMessage().contains("Ya existe la llave")) {
                        Notification n = Notification.show(
                                "La Presentacion " + this.depositos.getDescripcion()
                                        + " ya existe");
                        n.setPosition(Position.MIDDLE);
                        n.addThemeVariants(NotificationVariant.LUMO_ERROR);
                    }
        ...
        delete.addClickListener(e -> {
            try {
                ...
                binder.writeBean(this.depositos);
                depositosService.delete(this.depositos.getIdDeposito());
                clearForm();
                refreshGrid();
    ...
    private void searchFilter(GridListDataView<Depositos> dataView2) {
    ...
    private LitRenderer<Depositos> createDepositosRenderer() {
        return LitRenderer.<Depositos>of(
                "<vaadin-horizontal-layout style=\"align-items: center;\" theme=\"spacing\">"
                        + "  <vaadin-avatar name=\"${item.fullName}\"></vaadin-avatar>"
                        + "  <span> ${item.fullName} </span>"
                        + "</vaadin-horizontal-layout>")

                .withProperty("fullName", Depositos::getDescripcion);
    }
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        Optional<Short> depositosId = event.getRouteParameters().get(DEPOSITOS_ID)
                .map(Short::parseShort);
        if (depositosId.isPresent()) {
            Optional<Depositos> depositosFromBackend = depositosService.get(depositosId.get());
            if (depositosFromBackend.isPresent()) {
                populateForm(depositosFromBackend.get());
            } else {
                Notification.show(
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(DepositosView.class);
            }
        }
    }
    ...
    private void populateForm(Depositos value) {
        this.depositos = value;
        String topic = null;
        if (this.depositos != null) {
            topic = "depositos/" + this.depositos.getIdDeposito();
            avatarGroup.getStyle().set("visibility", "visible");
        } else {
            avatarGroup.getStyle().set("visibility", "hidden");
        }
        binder.setTopic(topic, () -> this.depositos);
