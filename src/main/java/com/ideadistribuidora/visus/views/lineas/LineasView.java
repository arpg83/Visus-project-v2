package com.ideadistribuidora.visus.views.lineas;
...
@Route(value = "10/:lineasID?/:action?(edit)")
public class LineasView extends Div implements BeforeEnterObserver {

    private final String LINEAS_ID = "lineasID";
    private final String LINEAS_EDIT_ROUTE_TEMPLATE = "10/%s/edit";

    private final Grid<Lineas> grid = new Grid<>(Lineas.class, false);
    ...
    private CollaborationBinder<Lineas> binder;

    private Lineas lineas;

    private final LineasService lineasService;
    GridListDataView<Lineas> dataView;

    public LineasView(LineasService lineasService) {
        ...
        // Configure Grid
        grid.addColumn(createLineasRenderer()).setHeader("Descripción").setAutoWidth(true);
        grid.addColumn(lineas -> lineas.getRubros().getDescripcion()).setHeader("Rubros").setAutoWidth(true);
        dataView = grid.setItems(lineasService.lineasList());
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                UI.getCurrent().navigate(
                        String.format(LINEAS_EDIT_ROUTE_TEMPLATE,
                                event.getValue().getIdLineas()));
            } else {
        ...
        // Configure Form
        binder = new CollaborationBinder<>(Lineas.class, userInfo);
        ...
        binder.setSerializer(Rubros.class,
                ...
                id -> lineasService.findById(Integer.parseInt(id)));
        ...
        save.addClickListener(e -> {
            ...
            try {
                ...
                binder.writeBean(this.lineas);
                lineasService.update(this.lineas);
                ...
                if (except.getCause().getCause() instanceof SQLException) {
                    ...
                    if (e1.getMessage().contains("Ya existe la llave")) {
                        Notification n = Notification.show(
                                "La Medida " + this.lineas.getDescripcion()
                                        + " ya existe");
                        n.setPosition(Position.MIDDLE);
                        n.addThemeVariants(NotificationVariant.LUMO_ERROR);
                    }
        ...
        delete.addClickListener(e -> {
            try {
                ...
                binder.writeBean(this.lineas);
                lineasService.delete(this.lineas.getIdLineas());
                clearForm();
                refreshGrid();
    ...
    private void searchFilter(GridListDataView<Lineas> dataView2) {
    ...
    private LitRenderer<Lineas> createLineasRenderer() {
        return LitRenderer.<Lineas>of(
                "<vaadin-horizontal-layout style=\"align-items: center;\" theme=\"spacing\">"
                        + "  <vaadin-avatar name=\"${item.fullName}\"></vaadin-avatar>"
                        + "  <span> ${item.fullName} </span>"
                        + "</vaadin-horizontal-layout>")

                .withProperty("fullName", Lineas::getDescripcion);
    }
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        Optional<Short> lineasId = event.getRouteParameters().get(LINEAS_ID)
                .map(Short::parseShort);
        if (lineasId.isPresent()) {
            Optional<Lineas> lineasFromBackend = lineasService.get(lineasId.get());
            if (lineasFromBackend.isPresent()) {
                populateForm(lineasFromBackend.get());
            } else {
                Notification.show(
    ...
    private void createEditorLayout(SplitLayout splitLayout) {
        ...
        rubros.setPlaceholder("Seleccione Depósito");
        rubros.setItems(lineasService.getAllRubros());
    ...
    private void populateForm(Lineas value) {
        this.lineas = value;
        String topic = null;
        if (this.lineas != null) {
            topic = "lineas/" + this.lineas.getIdLineas();
            avatarGroup.getStyle().set("visibility", "visible");
        } else {
            avatarGroup.getStyle().set("visibility", "hidden");
        }
        binder.setTopic(topic, () -> this.lineas);
